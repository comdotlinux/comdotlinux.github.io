...
class NavLink extends BaseComponent {
  ...
  handleSmoothScroll(href) {
    if (href.startsWith('#')) {
      ...
      if (target) {
        // Cache header height to avoid reflow on every scroll
        if (!this._cachedHeaderHeight) {
          this._cachedHeaderHeight = document.querySelector('site-header')?.offsetHeight || 80;
        }
        const targetPosition = target.offsetTop - this._cachedHeaderHeight;
